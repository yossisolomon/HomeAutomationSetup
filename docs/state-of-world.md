...
2. ✅ **IR code-set finder script** *(done — `scripts/find_ir_codeset.py` + `ir_codec.py` + `ir_match.py`)* — records via Broadlink IR-learn, searches the `ar_smart_ir` IR database to identify the right code set. Validated on Dana's office AC (RM4 mini `192.168.1.19`): resolved to **code 1622 — Tornado Master-22 X** (full temp matrix + swing: stop/hSwing/vSwing/swing), empirically confirmed on hardware. Main-AC run (RM4 Pro `192.168.1.18`) still pending.
