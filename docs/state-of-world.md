...
17. **CD — blacky safe auto-deploy** — poller on blacky: fetch `origin/main`, run HA
    ...
    ✅ **DONE + live on blacky (2026-06-18):** `scripts/cd_deploy.py` (stdlib-only, flock-guarded;
    pure `classify_changes`/`parse_secret` + thin main) + `tests/test_cd_deploy.py` (22 cases);
    `/etc/cron.d/cd-deploy` runs every 2 min as `yossi`; admin `cd_deploy_token` in `secrets.yaml`;
    `make check` annotated as superseded. **E2E verified:** a reload-safe PR (scenes.yaml) drove
    pull → `check_config` → `scene.reload` → Telegram, **no restart** (HA stayed healthy). The
    poller now auto-deploys every push to `main`.
